...
class UserMakesEvent(StaticLiveServerTestCase):
    ...
    def _make_new_event_last_month(self, name, day):
        day_div = self.browser.find_element(By.CLASS_NAME, f'last_month_day_{day}')
        action = ActionChains(self.browser)
        action.move_to_element(day_div).perform()
        new_event_button = self.browser.find_element(By.ID, f'last_month_new_event_button_{day}')
        new_event_button.click()
        name_input = self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['title_id'])
        submit_button = self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['submit_button'])
        name_input.send_keys(name)
    ...
    def _make_new_event_next_month(self, name, day):
        day_div = self.browser.find_element(By.CLASS_NAME, f'next_month_day_{day}')
        action = ActionChains(self.browser)
        action.move_to_element(day_div).perform()
        new_event_button = self.browser.find_element(By.ID, f'next_month_new_event_button_{day}')
        new_event_button.click()
        name_input = self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['title_id'])
        submit_button = self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['submit_button'])
        name_input.send_keys(name)
    ...
    def test_tiddlywinks_can_make_event(self):
        ...
        day_one = self.browser.find_element(By.CLASS_NAME, 'day_1')
        ...
        new_event_button = self.browser.find_element(By.ID, f'new_event_button_1')
        new_event_button.click()
        name_input = self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['title_id'])
        self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['time_id'])
        self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['date_id'])
        submit_button = self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['submit_button'])
        ...
        events = self.browser.find_elements(By.CLASS_NAME, 'day_1_event')
    ...
    def test_tiddlywinks_can_edit_event(self):
        ...
        event = self.browser.find_element(By.CLASS_NAME, 'day_1_event')
        event.click()
        # she sees a new form pop up with her old data
        edit_title_input = self.browser.find_element(By.ID, EDIT_EVENT_CLASS_IDS['title_id'])
        ...
        submit_button = self.browser.find_element(By.ID,EDIT_EVENT_CLASS_IDS['submit_button'])
        submit_button.click()
    ...
    def test_tiddlywinks_can_delete_an_event(self):
        ...
        event = self.browser.find_element(By.CLASS_NAME, 'day_1_event')
        event.click()
        delete_button = self.browser.find_element(By.ID, 'delete_button')
        delete_button.click()
        # the form dissappears
        bad_name_input = self.browser.find_element(By.ID, EDIT_EVENT_CLASS_IDS['title_id'])
        self.assertRaises(ElementNotInteractableException, bad_name_input.send_keys, 'something')
        # she doesn't see her event anymore
        events_on_1 = self.browser.find_elements(By.CLASS_NAME, 'day_1_event')
    ...
    def test_tiddlywinks_can_cancel_changes_or_new_event(self):
        ...
        event = self.browser.find_element(By.CLASS_NAME, 'day_5_event')
        event.click()
        # she don't wanna edit it so she clicks cancel
        cancel_button = self.browser.find_element(By.ID, 'edit_cancel_button')
        cancel_button.click()
        # the form dissappears
        bad_name_input = self.browser.find_element(By.ID, EDIT_EVENT_CLASS_IDS['title_id'])
        self.assertRaises(ElementNotInteractableException, bad_name_input.send_keys, 'something')
        # she decides to make a new event
        day_div = self.browser.find_element(By.CLASS_NAME, f'day_4')
        action = ActionChains(self.browser)
        action.move_to_element(day_div).perform()
        new_event_button = self.browser.find_element(By.ID, f'new_event_button_4')
        new_event_button.click()
        self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['title_id'])
        # she gives up because life is hard and decides to cancel the event
        cancel_button = self.browser.find_element(By.ID, 'new_cancel_button')
        cancel_button.click()
        bad_name_input = self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['title_id'])
        self.assertRaises(ElementNotInteractableException, bad_name_input.send_keys, 'something')
        # she logs out
    ...
    def test_tiddlywinks_can_make_plan_for_month_out(self):
        ...
        self.assertEqual(month_name.text, _get_month_name(self.curr_date.month))
        ...
        events_on_the_fifth = self.browser.find_elements(By.CLASS_NAME, 'day_5_event')
    ...
    def test_tiddlywinks_can_make_event_for_last_year(self):
        ...
        self.assertEqual(month_name.text, _get_month_name(self.curr_date.month))
        self.assertEqual(f'{self.curr_date.year}', year_num.text)
    ...
    def test_tiddlywinks_can_make_event_on_greyed_out_next_month_days(self):
        ...
        day_div = self.browser.find_element(By.CLASS_NAME, f'next_month_day_1')
        action = ActionChains(self.browser)
        action.move_to_element(day_div).perform()
        new_event_button = self.browser.find_element(By.ID, f'next_month_new_event_button_1')
        new_event_button.click()
        # She sees the date is for next month
        date_input = self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['date_id'])
        self.assertEqual(f'{self.curr_date.year}-{self.curr_date.month + 1}-1', date_input.get_attribute('value'))
        ...
        events = self.browser.find_elements(By.CLASS_NAME, 'next_day_1_event')
        self.assertEqual(len(events), 1)
        # She makes sure the event got added to the right day
        event = self.browser.find_element(By.CLASS_NAME, 'next_day_1_event')
        event.click()
    ...
    def test_tiddlywinks_can_make_and_edit_event_on_greyed_out_last_month_days(self):
        ...
        action = ActionChains(self.browser)
        action.move_to_element(day_div).perform()
        new_event_button = self.browser.find_element(By.ID, f'last_month_new_event_button_31')
        new_event_button.click()
        # She sees the date is for next month
        date_input = self.browser.find_element(By.ID, NEW_EVENT_CLASS_IDS['date_id'])
        ...
        submit_button.click()
        # she sees her event on screen
        events = self.browser.find_elements(By.CLASS_NAME, 'last_day_31_event')
        self.assertEqual(len(events), 1)
        # She makes sure the event got added to the right day
        event = self.browser.find_element(By.CLASS_NAME, 'last_day_31_event')
        event.click()
        # she realizes she needs to change the event
        edit_title_input = self.browser.find_element(By.ID, EDIT_EVENT_CLASS_IDS['title_id'])
        edit_title_input.clear()
    ...
    def test_tiddly_winks_can_make_and_edit_event_around_year_end(self):
        ...
        left_arrow = self.browser.find_element(By.ID, 'left_month')
        left_arrow.click()
        event = self.browser.find_element(By.CLASS_NAME, 'day_31_event')
        event.click()
        edit_title_input = self.browser.find_element(By.ID, EDIT_EVENT_CLASS_IDS['title_id'])
